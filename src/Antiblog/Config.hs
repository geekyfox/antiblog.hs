...
import Data.HashMap.Strict(toList,lookup,HashMap)
import Data.Ini(unIni,readIniFile)
...
instance Config Remote where
    ...
    loadConfig = readConfig ".antisync/antisync.conf" >>== (sequenceA . map digest)
...
instance Config Local where
    ...
    loadConfig = readConfig ".antiblog/antiblog.conf" >>== (sequenceA . map digest)
...
loadOrDie n = case n of
    Just n -> exposeOrDie <$> (loadConfig >>== select n)
...
readConfig :: FilePath -> IO (Outcome [(SystemName, HashMap Text Text)])
readConfig suffix = do
    ...
    if ex
        then
            (map (first shapeshift) . toList . unIni) <$$> fromEither <$> readIniFile fname
        else
            return (OK [])
